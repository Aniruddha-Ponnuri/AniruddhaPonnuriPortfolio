    ...
    return (
      <div className="surface-card space-y-4 rounded-xl p-5">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="h-10 rounded-md bg-muted md:col-span-2 animate-pulse" />
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {availableTags && availableTags.length > 0 && availableTags.slice(0, 20).map((tag) => (
            <Badge
              ...
              className="cursor-pointer transition-[transform,background-color,color] duration-140 ease-[var(--ease-out)] active:scale-[0.97]"
        ...
        {selectedTags && selectedTags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {selectedTags.map((tag) => (
              <Badge key={tag} className="flex items-center gap-1 px-2.5 py-1">
                {tag}
                <X
